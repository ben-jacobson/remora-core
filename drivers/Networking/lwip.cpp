...
int32_t lwip_handle::send_lwip(uint8_t sn, uint8_t *buf, uint16_t len)
{
    uint8_t tmp = 0;
    uint16_t freesize = 0;
...
int32_t lwip_handle::recv_lwip(uint8_t sn, uint8_t *buf, uint16_t len)
{
    uint8_t head[2];
    uint16_t pack_len = 0;
...
err_t lwip_handle::netif_output(struct netif *netif, struct pbuf *p)
{
    uint32_t send_len = 0;
    uint32_t tot_len = 0;
    ...
    if (active_instance)
        send_len = active_instance->send_lwip(0, tx_frame, tot_len);

    return ERR_OK;
}

void lwip_handle::netif_link_callback(struct netif *netif)
{
    printf("netif link status changed %s\n", netif_is_link_up(netif) ? "up" : "down");
}

void lwip_handle::netif_status_callback(struct netif *netif)
{
    printf("netif status changed %s\n", ip4addr_ntoa(netif_ip4_addr(netif)));
}

err_t lwip_handle::netif_initialize(struct netif *netif)
{
    netif->linkoutput = lwip_handle::netif_output;
...
uint32_t lwip_handle::ethernet_frame_crc(const uint8_t *data, int length)
{
